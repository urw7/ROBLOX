...
local versionId = 10245 -- this gets edited once the number changes.
...
if game:GetService("ReplicatedStorage").wkspc.VersionID.Value ~= versionId then
	...
	local ChatSpam = getfenv().Message or "You."
	local Target

	local SwitchToKnifeWeapons = {
		"Electric Revoler", --[[Because if you kill too many people it bans you :skull:]] "Paintball", "Dispenser", "Bow", "Flamethrower", "Acid", "Cannon", "Launcher", "RPG", "Water Balloon", "Presents", "Bomb", "Explosive Barrel", "Knife", "Concussion Rifle" -- <-- doesnt ban but slow af on concussion mode
	}
...
------------------------------------------------------------------------------------------------------------
        ...
        if game:GetService("ReplicatedStorage").wkspc.Status.LastGamemode.Value == "Monkey Business" then
			HopServers()
			return
		end

        if game:GetService("ReplicatedStorage").wkspc.Status.LastGamemode.Value == "Randomizer" then
			HopServers()
			return
		end

		if game:GetService("ReplicatedStorage").wkspc.Status.LastGamemode.Value == "OddBall" then
			HopServers()
			return
		end
-------------------------------------------------------------------------------------------------------------
